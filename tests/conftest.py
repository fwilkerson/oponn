...
def reset_service():
    ...
    # Since we use a real singleton for background server support, 
    # we manually clear its repository instead of overriding the dependency.
    _ballot_repo = _ballot_service.repository
    # We clear the internal dicts of the InMemory repository
    _ballot_repo.ballots_db.clear()
    _ballot_repo.votes_db.clear()
    _ballot_repo.ballot_id_counter = 0
    _ballot_service.sse_queues.clear()
