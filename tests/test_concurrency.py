...
async def test_high_concurrency_mixed_options():
    ...
    bc = BallotCreate(measure="Mixed Concurrent", options=["A", "B"], allow_write_in=True)
    ballot = await service.create_ballot(bc)
    ...
    async def vote_write_in(i):
        await service.record_vote(ballot.ballot_id, Vote(option=f"WriteIn_{i}", is_write_in=True))
